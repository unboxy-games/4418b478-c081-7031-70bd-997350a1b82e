(function (global, factory) {
    ...
    var SetupSyncFlags = function (state, config) {
        ...
        if (typeof (config) === 'boolean') {
            ...
            state.syncCameraFilter = GetValue$G(config, 'syncCameraFilter', true);
            state.syncDisplayList = GetValue$G(config, 'syncDisplayList', true);
        }
    ...
    */
    ...
    var DrawBounds$2 = function (gameObjects, graphics, config) {
        ...
        if (typeof (config) === 'number') {
            ...
            fillAlpha = GetValue$E(config, 'fillAlpha');
            padding = GetValue$E(config, 'padding');
            includeParent = GetValue$E(config, 'includeParent');
        }
    ...
    */
    ...
    var TapChild = function (config) {
        var tapConfig = GetValue$6(config, 'tap', undefined);
        if (tapConfig === false) {
            return;
    ...
    var PressChild = function (config) {
        var pressConfig = GetValue$5(config, 'press', undefined);
    ...
    var SwipeChild = function (config) {
        var swipeConfig = GetValue$4(config, 'swipe', undefined);
    ...
    var SetChildrenInteractive = function (gameObject, config) {
        ...
        if (GetValue$3(config, 'dropZone', false)) {
            gameObject.input.dropZone = true;
        }

        gameObject._childrenInteractive = {
            targetSizers: GetValue$3(config, 'targets', [gameObject]),
            targetMode: GetValue$3(config, 'targetMode', 'parent'),
            eventEmitter: GetValue$3(config, 'eventEmitter', gameObject),
            eventNamePrefix: GetValue$3(config, 'inputEventPrefix', 'child.')
        };
    ...
    class Base extends ContainerLite {
        constructor(scene, x, y, minWidth, minHeight, config) {
            ...
            var origin = GetValue$2(config, 'origin', 0.5);
            var originX = GetValue$2(config, 'originX', origin);
            var originY = GetValue$2(config, 'originY', origin);
            this.setOrigin(originX, originY);

            this.setMinSize(minWidth, minHeight);
            this.setName(GetValue$2(config, 'name', ''));
            this.rexSizer = {};
            this.space = {};
            ...
            this.setInnerPadding(GetValue$2(config, 'space', 0));

            var draggable = GetValue$2(config, 'draggable', false);
            if (draggable) {
                this.setDraggable(draggable);
            }

            this.setSizerEventsEnable(GetValue$2(config, 'sizerEvents', false));
            this.setDirty(true);

            if (GetValue$2(config, 'enableLayer', false)) {
                this.enableLayer();
            }
        }
    ...
    var Add = function (
        ...
        if (proportion === null) {
            ...
            proportion = GetValue$1(config, 'proportion', undefined);
            ...
            if (!gameObject.isRexSizer) {
                minWidth = GetValue$1(config, 'minWidth', undefined);
                minHeight = GetValue$1(config, 'minHeight', undefined);
            }

            fitRatio = GetValue$1(config, 'fitRatio', 0);  // width/height

            offsetX = GetValue$1(config, 'offsetX', 0);
            offsetY = GetValue$1(config, 'offsetY', 0);
            offsetOriginX = GetValue$1(config, 'offsetOriginX', 0);
            offsetOriginY = GetValue$1(config, 'offsetOriginY', 0);
        }
